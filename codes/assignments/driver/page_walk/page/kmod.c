...
/**Global variables*/
char *devname; //contains device name int majNo; //major number
...
long myIoctl(struct file *fp, unsigned int request, unsigned long num)
{
	...
	unsigned long var = 0;
	struct address_space *ptr2 = NULL;
	struct page *ptr1 = NULL;

	ptr2 = current -> mm -> mmap -> vm_file -> f_mapping;
	var = current -> mm -> mmap -> vm_pgoff;
	ptr1 = find_get_page(ptr2, var);
	ptr = kmap(ptr1);
	printk("var %lx\n", ptr);
	*ptr = 100;
	return 0;
